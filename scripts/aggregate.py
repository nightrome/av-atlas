...
def main():
    ...
    stats = {
        ...
        # labels. Those used to be generated by truncating the name to three
        # letters, which collided outright on real data: "SA" stood for both
        # Saudi Arabia and South Africa, "Ind" for both India and Indonesia,
        # and "Aus" for both Australia and Austria -- three pairs of cells a
        # reader could not tell apart. Shipped from the same table that
        # produced the names in the first place, so the two cannot disagree.
        # Several codes map to one name on purpose (Hong Kong and Macao are
        # merged into China above); first code wins, which is the country's
        # own code in every such case.
        "country_codes": {name: code for code, name in reversed(list(COUNTRY_NAMES.items()))},
        ...
        # Institution -> country, for the detail pages' Country columns.
        #
        # Those columns used to read an author's own flat `countries` list
        # while the neighbouring Institution column picked an institution by
        # year overlap -- two independent lookups over two different
        # structures, which produced visibly self-contradicting rows on real
        # data ("United States / University of Tubingen", "Singapore /
        # Berkeley AIR"). Shipping the mapping lets those pages derive the
        # country FROM the institution they are already showing, so the two
        # cells cannot disagree by construction. Restricted to institutions
        # that actually appear in the corpus, not the whole 450-entry file.
        "institution_countries": {
            inst: institution_countries[inst]
            for inst in inst_citations if inst in institution_countries
        },
